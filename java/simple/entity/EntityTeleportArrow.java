...
public class EntityTeleportArrow extends Entity implements IProjectile
{
    ...
    public EntityTeleportArrow(World worldIn, EntityLivingBase shooter, EntityLivingBase p_i1755_3_, float p_i1755_4_, float p_i1755_5_)
    {
        super(worldIn);
        this.renderDistanceWeight = 10.0D;
            ...
            if (movingobjectposition != null)
            {
                ...
                else
                {
                    ...
                    if(shootingEntity != null) {
                        ...
                        double portY = yTile;
                        double portZ = zTile;
                        System.out.println("Printing arrow position");
                        System.out.println("x: "+String.valueOf(portX)+" y: "+String.valueOf(portY)+" z: "+String.valueOf(portZ));
                        double distance = Math.sqrt(Math.pow((this.posX-player.posX), 2) + Math.pow((this.posY-player.posY), 2) + Math.pow((this.posZ-player.posZ), 2));
                        System.out.println("Printing distance");
                        System.out.println(String.valueOf(distance));
                        double directionX = -(this.posX - player.posX)/distance;
                        double directionY = -(this.posY - player.posY)/distance;
                        double directionZ = -(this.posZ - player.posZ)/distance;
                        System.out.println("Printing direction");
                        System.out.println("x: "+String.valueOf(directionX)+" y: "+String.valueOf(directionY)+" z: "+String.valueOf(directionZ));
                        ...
                        for(int iter = 0; iter < 100; iter++) {
                            ...
                            if(isAirOrLiquid(worldObj, (int)portX, (int)portY, (int)portZ) && isAirOrLiquid(worldObj, (int)portX, ((int)portY)-1, (int)portZ)) {
                                player.setPositionAndUpdate(portX, portY, portZ);
    ...
    public boolean isAirOrLiquid(World world, int x, int y, int z) {
        ...
        return world.isAirBlock(pos) || world.getBlockState(pos).getBlock().getMaterial() == Material.water || world.getBlockState(pos).getBlock().getMaterial() == Material.lava;
    }
     ...
     */
    public static DamageSource causeArrowDamage(simple.entity.EntityTeleportArrow par0EntityArrow, Entity par1Entity)
    {
